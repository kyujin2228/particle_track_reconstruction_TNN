...
class EdgeNet(Module):
    def __init__(self):
        ...
        if (dp_noise!=None):
            ...
            # self.exp_layer = TorchConnector(self.qnn)
        else: 
        ...
        # params_iec=ParameterVector('θ',length=self.n_qubits)
        # for i in range(self.n_qubits):
        #     self.IEC_circuit.ry(params_iec[i],i)

        # self.PQC_circuit = QuantumCircuit(self.n_qubits)
        # params_pqc=ParameterVector('l',length=self.n_qubits)
        # for i in range(self.n_qubits):
        #     self.PQC_circuit.ry(params_pqc[i],i)

        # self.model_circuit=QuantumCircuit(self.n_qubits)
        # self.model_circuit.compose(self.IEC_circuit,inplace=True)
        # self.model_circuit.compose(self.PQC_circuit,inplace=True)
...
class NodeNet(Module):
    def __init__(self):
        ...
        if (dp_noise!=None):
            ...
            error2 = depolarizing_error(dp_noise, 2)
            noise_model.add_basis_gates('ry')
            noise_model.add_all_qubit_quantum_error(error1, [ 'id', 'ry', 'sx', 'u1', 'u2', 'u3'])
            noise_model.add_all_qubit_quantum_error(error2, [ 'cx'])
            sim_noise=AerSimulator(noise_model=noise_model)
            self.model_circuit = transpile(self.model_circuit, sim_noise)
