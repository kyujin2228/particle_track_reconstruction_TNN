...
if __name__ == '__main__':
    ...
    # print model summary
    # print(pytorch_model_summary.summary(model,[map2angle(X).shape,Ri.shape,Ro.shape],show_input=True))

    # Log initial parameters if new run
    if config['run_type'] == 'new_run':    
        if config['log_verbosity']>=2:
            # print(model.parameters())
            log_parameters(config['log_dir'], model.parameters())
        epoch_start = 0
    ...
    loss_func = BCELoss()
    optimizer = getattr(optim,config['optimizer'])(model.parameters(), lr=config['lr_c'])
    ...
    for epoch in range(epoch_start, config['n_epoch']):
        ...
        for n_step in range(config['n_train']//config['batch_size']):
            ...
            # iterate a step
            loss = batch_train_step(n_step)  # Calculate loss
            # end timer
            ...
            print(
                ...
                %(epoch+1, n_step+1, loss.detach().numpy() ,t / 60, t % 60)
                )
            # Start logging 
            ...
            with open(config['log_dir']+'summary.csv', 'a') as f:
                f.write(
                    '%d, %d, %f, %f\n' \
                    %(epoch+1, n_step+1, loss.detach().numpy(), t)
                    )
	       # Log parameters
            if config['log_verbosity']>=2:
                log_parameters(config['log_dir'], model.parameters())
