...
def measure_TNN_qiskit(n_measurements=3):
    observable1 = SparsePauliOp.from_list([("Z" + "I" * 5, 1)]) #0
    observable2 = SparsePauliOp.from_list([("I"+"Z" + "I" * 4, 1)]) #2
    observable3 = SparsePauliOp.from_list([("I"*2+"Z" + "I"*3, 1)]) #2
    # observable3 = SparsePauliOp.from_list([("I"*7+"Z" , 1)]) #7
    # observable1 = SparsePauliOp.from_list([("Z" + "I" * 7, 1)]) #0
    # observable2 = SparsePauliOp.from_list([("I"*2+"Z" + "I" * 5, 1)]) #2
    # observable3 = SparsePauliOp.from_list([("I"*7+"Z" , 1)]) #7
    return observable1,observable2,observable3
###############################################################
###############################################################
#############    INFORMATION ENCODING CIRCUITS    #############
###############################################################
###############################################################
def simple_encoding_y_qiskit(circuit, n_qubits=4):
...
def U_con3(circuit,qubits,params):
    circuit.append(cirq.ry(params[0])(qubits[0]))
    circuit.append(cirq.ry(params[1])(qubits[1]))
    circuit.append(cirq.CNOT(qubits[1],qubits[0]))
...
def U_con3_qiskit(circuit,qubits,params):
    circuit.ry(params[0],qubits[0])
    circuit.ry(params[1],qubits[1])
    circuit.cx(qubits[1],qubits[0])
    circuit.ry(params[2],qubits[0])
    circuit.ry(params[3],qubits[1])
...
def conv_layer1_qiskit(unitary,circuit,params):
    if unitary=='U_con3_qiskit':
        unitary=U_con3_qiskit
    for i in range(4):
        qubits=[2*i,2*i+1]
        unitary(circuit,qubits,params)
    for i in range(4):
        if i != 3:
            qubits=[2*i+1,2*i+2]
            unitary(circuit, qubits,params)
            ...
            qubits=[7,0]
            unitary(circuit, qubits,params)
def conv_layer1_qiskit_6(unitary,circuit,params):
    if unitary=='U_con3_qiskit':
        unitary=U_con3_qiskit
    for i in range(3):
        qubits=[2*i,2*i+1]
...
def conv_layer2(unitary,circuit,qubits,params):
    ...
    unitary(circuit, qubit,params)
def conv_layer2_qiskit(unitary,circuit,params):
    if unitary=='U_con3_qiskit':
        unitary=U_con3_qiskit
    # 2-3,6-7 wires
    ...
    qubit=[qubits[7],qubits[2]]
    unitary(circuit, qubit,params)
def conv_layer2_qiskit_6(unitary,circuit,params):
    if unitary=='U_con3_qiskit':
        unitary=U_con3_qiskit
    qubits=[i for i in range(6)]
    qubit=[0,2]
...
def conv_layer3(unitary,circuit,qubits,params):
    ...
    # 6-7 wires
    unitary(circuit, qubits[6:8],params)
def conv_layer3_qiskit(unitary,circuit,params):
    if unitary=='U_con3_qiskit':
        unitary=U_con3_qiskit
    # 6-7 wires
    qubits=[i for i in range(8)]
    unitary(circuit, qubits[6:8],params)
def conv_layer3_qiskit_6(unitary,circuit,params):
    if unitary=='U_con3_qiskit':
        unitary=U_con3_qiskit
    # 6-7 wires
    qubits=[i for i in range(8)]
...
def TNN_con3_pqc(circuit, qubits, n_layers=1, n_qubits=8, symbol_offset=0):
    n_params=4
    params = sympy.symbols('theta{}:{}'.format(symbol_offset, symbol_offset + n_params*3))
    params_each=[]
    for i in range(3):
        params_each.append(params[3*i:3*i+n_params])
    conv_layer1(U_con3,circuit,qubits,params_each[0])
    conv_layer2(U_con3,circuit,qubits,params_each[1])
    conv_layer3(U_con3,circuit,qubits,params_each[2])
...
def TNN_con3_qiskit(circuit,n_layers=1, n_qubits=8, symbol_offset=0):
    n_params=4
    params=ParameterVector('l',length=n_params*3)
    # qubits=[i for i in range(n_qubits)]
    params_each=[]
    for i in range(3):
        params_each.append(params[3*i:3*i+n_params])
    conv_layer1_qiskit(U_con3_qiskit,circuit,params_each[0])
    conv_layer2_qiskit(U_con3_qiskit,circuit,params_each[1])
    conv_layer3_qiskit(U_con3_qiskit,circuit,params_each[2])
    return params

def TNN_con3_qiskit_6(circuit,n_layers=1, n_qubits=8, symbol_offset=0):
    n_params=4
    params=ParameterVector('l',length=n_params*3)
    # qubits=[i for i in range(n_qubits)]
    params_each=[]
    for i in range(3):
        params_each.append(params[3*i:3*i+n_params])
    conv_layer1_qiskit_6(U_con3_qiskit,circuit,params_each[0])
    conv_layer2_qiskit_6(U_con3_qiskit,circuit,params_each[1])
    conv_layer3_qiskit_6(U_con3_qiskit,circuit,params_each[2])
    return params

def TNN_con3_qiskit_test(circuit,n_layers=1, n_qubits=8, symbol_offset=0):
    ...
    return params
###############################################################
# def qc10_8_pqc(circuit, qubits, n_layers=1, n_qubits=8, symbol_offset=0):
